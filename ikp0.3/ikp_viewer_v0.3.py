#!/usr/bin/env python3
"""
IKP Viewer v0.4 (patched to use ikp_core)
Lightweight runtime for .ikp files
- Uses core for YAML, validation, interpolation, action execution
"""
...
class IKPViewer(tk.Tk):
    ...
    def _get_vars_map(self):
        return {k: (v.get() if hasattr(v, "get") and callable(v.get) else (v() if callable(v) else v)) for k, v in self.vars.items()}
